...
void printExpectedResult(
    const std::expected<bool, inventory::InventoryError>& result)
{
    if(result)
    {
...
int main()
{
    ...
    bag.addItemOrThrow(
        0, inventory::ItemFactory::makePolymorphic(createdItems[0]));
    bag.addItemOrThrow(
        1, inventory::ItemFactory::makePolymorphic(createdItems[1]));
    bag.addItemOrThrow(
        2, inventory::ItemFactory::makePolymorphic(createdItems[2]));
    ...
    printExpectedResult(bag.addItem(
        0, std::make_unique<inventory::Consumable>("another_potion", 0.3, 10)));

    for(std::size_t slot = 3; slot < bag.capacity(); ++slot)
    {
        bag.addItemOrThrow(
            slot, std::make_unique<inventory::Consumable>("ration", 0.2, 5));
    }
